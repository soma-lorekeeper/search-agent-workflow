...
# ---------- 설정값 ----------
#
# 아래 기본값은 **실측으로 정했다**(tests/test_llm_smoke.py, 2026-08-16 관측):
#
#   gpt-5.6-luna          TPM 200,000   RPM 500
#   text-embedding-3-small TPM 1,000,000 RPM 3,000
#   임베딩 호출 지연        평균 512ms (300~830ms)
#
# 세마포어는 **거절이 아니라 대기**라 낮게 잡아도 요청이 실패하지 않는다. 그래서
# 안전한 쪽으로 잡고, 처리량이 문제가 되면 환경변수로 올린다.

# LLM은 **TPM이 병목**이다(RPM은 여유 — N=4에 지연 20초면 약 12 RPM, 한도 500).
# 기준은 임베딩과 같다: 지속 소비율 < 충전율(200,000/60 ≈ 3,333 토큰/초).
#
# 추출 호출 실측(2026-08-16, 실제 프롬프트 + 실제 원고 조각):
#   입력 4,924 + 출력 2,530 = 8,052 토큰 / 호출,  지연 20.1초
#   N=4 지속 소비율 1,303 토큰/초 = 충전율의 39%
#
...
#
#   슬롯 4개가 전부 추출 → 1,600/초 (48%)   여유
#   슬롯 4개가 전부 판정 → 5,400/초 (162%)  초과 (27k 기준. 긴 회차면 더 크다)
#
# 6으로 올리면 추출 72%는 괜찮지만 판정이 243%가 된다. 꼬리 위험이 이득보다 커서 4를 둔다.
# 넘치면 429 백오프가 받아내지만 인덱싱 경로의 백오프가 20/40/80초라 로컬에서 줄 서는
# 편이 훨씬 싸다.
_MAX_CONCURRENCY = int(os.environ.get("LLM_MAX_CONCURRENCY", "4"))

# 임베딩은 반대로 **RPM이 병목**이다. 호출당 토큰이 20~30개뿐이라 TPM 1M은 닿지 않고,
# 대신 탐지 1건이 claim×4채널까지 던져 요청 수가 크다.
#
# 기준은 "한도의 몇 %"가 아니라 **충전 속도**다. RPM 3,000은 60초 창이 아니라 초당 50개씩
# 차는 연속 충전 버킷이다(실측 reset이 3ms인 것이 그 증거 — "만수위 복귀까지"의 뜻이라
# 조금 쓰면 즉시 찬다). 그래서 지속 소비율이 충전율을 넘으면 버킷이 서서히 말라 429가 난다.
#
# 동시성별 실측 처리량(2026-08-16):
#   N= 8 → 15.2/초   N=16 → 25.2/초   N=24 → 36.7/초   N=32 → 72.4/초
#   지연은 N=32까지도 안 늘었다(456→325ms) — 서버가 아니라 우리 동시성이 유일한 변수다.
#
# 24를 고른 이유: 36.7/초 < 충전 50/초라 **지속 가능한 마지막 구간**이다. 32는 버스트로는
# 되지만 탐지가 연달아 들어오면 마른다. 남는 13/초는 인덱싱의 임베딩(청크·사실) 몫이다 —
# 같은 3,000 버킷을 나눠 쓴다.
_EMBEDDING_CONCURRENCY = int(os.environ.get("EMBEDDING_MAX_CONCURRENCY", "24"))

# 콜드 스타트 가정값. 첫 응답 헤더가 오면 즉시 실제 값으로 교체된다 — 한도가 아니라
# "아직 아무것도 못 본 동안의 임시값"이다. 실측한 luna 값과 같게 두되, 임베딩 버킷은
# 이보다 훨씬 크므로(1M/3000) 첫 응답 전까지만 과소평가한다(안전한 방향).
_ASSUMED_TPM = int(os.environ.get("LLM_ASSUMED_TPM", "200000"))
_ASSUMED_RPM = int(os.environ.get("LLM_ASSUMED_RPM", "500"))
