...
- 작품·사물을 저작/제작/열독한 인물은 그 역할을 CharacterState(예: '탑의 문의 저자') + ABOUT→Item으로
  표현한다. 이런 역할은 사람 사이의 관계가 아니라 그 사물에 대한 각자의 역할이므로, 두 인물을 서로
  잇지 말고 각자를 그 Item에 잇는다. 단, 이 역할과 그 사물/작품 Item도 아래 '비중 필터'를 통과할
  때만 만든다.
...
- 인물↔인물의 서사적 관계(사제·동맹·적대·혈연·연인·동료 등)도 CharacterState + ABOUT→Character로
  표현한다(단순 동반 등장만으로는 만들지 않음). 관계는 두 사람 각자의 상태이므로 **양쪽을 모두
  만든다** — A가 B의 스승이면 A에게 'B의 스승'을, B에게 'A의 제자'를 각각 만들고 서로를 ABOUT으로
  가리키게 한다. 한쪽만 만들면 다른 인물을 조회했을 때 그 관계가 보이지 않는다.
